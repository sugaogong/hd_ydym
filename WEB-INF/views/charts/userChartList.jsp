...
<html>
	...
	<body>
    	<ul class="nav nav-tabs sys-font">
			<li class="active"><a href="">注册数量统计分析报表</a></li>
		</ul>
  	...
  	<script>
  		...
  		function dataQuery(){
  			...
  			if(dataType == 1){
	  			...
	  			$.ajax({
	  				type:"post",
	  				url:"${ctx}/api/data/getUserDataByDate",
	  				data:{
	  					userId:userId,
	  					beginDate:beginDate,
	  					endDate:endDate,
	  				...
	  				success:function(data){
	  					...
	  					if(data.resultCode == 0){
	  						...
	  						for(var i in data.resultData){
	  							...
	  							txt += '	<td>'+ data.resultData[i].activCount +'</td>';
	  							txt += '	<td>'+ data.resultData[i].autoCount +'</td>';
	  							txt += '	<td>'+ data.resultData[i].totlCount +'</td>';
	  							txt += '</tr>';
	  							...
	  							totlseriesNums = totlseriesNums + parseInt(data.resultData[i].totlCount);
	  						}
  							txt += '<tr>';
  							txt += '	<td>总计</td>';
	  			...
	  			$.ajax({
	  				type:"post",
	  				url:"${ctx}/api/data/getUserDataByUser",
	  				data:{
	  					userId:userId,
	  					beginDate:beginDate,
	  					endDate:endDate,
	  					...
	  					if(data.resultCode == 0){
	  						...
	  						for(var i in data.resultData){
  								...
  								var arr = {
  	  									...
  	  									value:data.resultData[i].totlCount,
  	  									userDefined: data.resultData[i].id
  		  							}
  								var arr1 = {
  	  									name: data.resultData[i].name,
  	  									value:data.resultData[i].activCount,
  	  									userDefined: data.resultData[i].id
  		  							}
  								var arr2 = {
  	  									name: data.resultData[i].name,
  		...
  		function exportForm(){
  			...
  			var endDate = $("#finance_EndTime").val();
  			if(dataType == 1){ //日期
  				location.href = "${ctx}/api/data/exportUserDataByDate?userId="+ userId + "&beginDate=" + beginDate + "&endDate=" +endDate;
  			}else{ //组织
  				location.href = "${ctx}/api/data/exportUserDataByUser?userId="+ userId + "&beginDate=" + beginDate + "&endDate=" +endDate;
  			}
  		}
  		//日期
