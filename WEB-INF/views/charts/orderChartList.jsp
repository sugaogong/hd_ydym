...
<html>
	...
	<body>
    	<ul class="nav nav-tabs sys-font">
			<li class="active"><a href="">订单统计分析报表</a></li>
		</ul>
  	...
  	<script>
  		...
  		function dataQuery(){
  			...
  			if(dataType == 1){
	  			...
	  			$.ajax({
	  				type:"post",
	  				url:"${ctx}/api/data/getOrderDataByDate",
	  				data:{
	  					userId:userId,
	  					beginDate:beginDate,
	  					endDate:endDate,
	  					...
	  					if(data.resultCode == 0){
							...
							txt += '		<td></td>';					
	  						...
	  						for(var i in data.resultData){
	  							...
	  							totlseries.push(data.resultData[i].totalCount);
	  							activseries.push(data.resultData[i].comCount);
	  							/* autoseries.push(data.resultData[i].autoCount); */
	  							txt += '<tr>';
	  							txt += '	<td>'+ data.resultData[i].createDate.split(" ")[0] +'</td>';
	  							txt += '	<td>'+ data.resultData[i].comCount +'</td>';
	  							txt += '	<td>'+ data.resultData[i].totalCount +'</td>';
	  							txt += '	<td>'+ data.resultData[i].totalAmount +'</td>';
	  							txt += '</tr>';
	  							activseriesNums = activseriesNums + parseInt(data.resultData[i].comCount);
	  							autoseriesNums = autoseriesNums + parseInt(data.resultData[i].totalCount);
	  							totlseriesNums = totlseriesNums + parseFloat(data.resultData[i].totalAmount.replace(/\,/, ''));
	  						}
  							txt += '<tr>';
  							txt += '	<td>总计</td>';
  							txt += '	<td>'+ activseriesNums +'</td>';
  							txt += '	<td>'+ autoseriesNums +'</td>';
  							txt += '	<td>'+ parseFloat(totlseriesNums.toFixed(2)) +'</td>';
  							txt += '</tr>';
	  						txt += '</tbody>';
	  			...
	  			$.ajax({
	  				type:"post",
	  				url:"${ctx}/api/data/getOrderDataByUser",
	  				data:{
	  					userId:userId,
	  					beginDate:beginDate,
	  					endDate:endDate,
	  					...
	  					if(data.resultCode == 0){
							...
							txt += '		<td></td>';					
	  						...
	  						for(var i in data.resultData){
  								...
  								/* var arr2 = {
  	  									name: data.resultData[i].name,
  	  									value:data.resultData[i].autoCount,
  	  									userDefined: data.resultData[i].id
  		  							}//data:['总订单数量','已完成数量'] */
  								totlseries.push(arr);
  		...
  		function exportForm(){
  			...
  			var endDate = $("#finance_EndTime").val();
  			if(dataType == 1){ //日期
  				location.href = "${ctx}/api/data/exportOrderDataByDate?userId="+ userId + "&beginDate=" + beginDate + "&endDate=" +endDate;
  			}else{ //组织
  				location.href = "${ctx}/api/data/exportOrderDataByUser?userId="+ userId + "&beginDate=" + beginDate + "&endDate=" +endDate;
  			}
  		}
  		//日期
